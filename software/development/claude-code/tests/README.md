...
| `test_cc_scrub.sh` | `cc-scrub.sh` F1 arm: calibration that fails loudly when a detector is broken or over-broad, the four rules against their documented false-positive baits, `NEW`-vs-`BASELINE` classification, corpus honesty, and the 0904b regression replayed against real refs (RESEARCH-1). |
| `test_git_hooks.sh` | The cc-scrub git hooks: `configure.sh` installs `pre-commit`/`pre-push` as symlinks into `canonical/hooks/`, idempotently, refusing to install over a foreign `core.hooksPath` and backing up an operator's own hook; each hook then blocks a seeded disclosure (driven through real `git commit` / stdin in git's pre-push format), passes a clean change, skips ref deletions, bounds a new remote ref against a published baseline, and fails CLOSED on a missing scrubber or an INCOMPLETE sweep (INFRA-59). |
| `test_cc_scrub_outbound.sh` | `cc-scrub-outbound.sh`: the register/typography rules and their ASCII baits, the bare 22-hex session id the F1 arm cannot see, the delegated F1 class (including that a delegate which fails calibration makes the run INCOMPLETE), outbound package completeness, and a sweep of the real staged packages (INFRA-62). |
